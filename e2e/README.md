...
The `ci-links` suite reads `tests/ci-links.json`, which is generated from the
...
changes. It defaults to the local dev server (`http://localhost:4321`); point it
at another site with `BASE_URL`:
...
- `tests/ci-links.spec.ts`: the broader CI link-audit. It reads
  `tests/ci-links.json` (generated by `utils/generate-ci-links.ts`) and renders
  every internal link in headless Chromium, asserting the HTTP status is < 400
  and that no browser console / page errors fired (captured by `fixtures.ts`).
